...
subtitle: "Lecture 2 & 3: Optimal Portfolio Choice"
footer: "Lecture 2 & 3: Optimal Portfolio Choice"
---
...
::: {.center}
![](Markowitz.png){fig-align="center"} Harry Markowitz Nobel Memorial Prize 1990
:::
...
B.  Efficient portfolios with two stocks

C.  $N$ risky stocks: the efficient frontier

D.  The "super-efficient" portfolio
...
::: {.center}
![](MVchoice.png){fig-align="center"}
:::

## Mean-variance utility {.smaller}

- Mean-variance analysis can be motivated with utility derived from the
  first two moments of wealth: $U(E(W),\text{Var}(W)),$ where $U$ is
  increasing in the expected wealth, $E(W)$, and decreasing in the
  variance of wealth, $\text{Var}(W).$

- A common specification is $$
  U(E(W),\text{Var}(W)) = E(W)-\frac{\gamma}{2} \text{Var}(W),
  $$ where $\gamma>0$ measures the aversion to variance
  relative to the preference for the mean. $\gamma$ is related to the
  coefficient of absolute risk aversion discussed in the previous
  lecture.

- In our simplified framework more risk aversion or lower risk tolerance
...
## Probability reminder {.smaller .dense}

- Computation of the covariance and correlation: Consider the random
  variables $X$, $Y$ with payments $x_i$, $y_i$, $i=1,\dots,N$:

$$\begin{aligned}
\operatorname{Covariance}\left(X,Y\right) & = \sigma_{X,Y} = E\left[\left(X-E\left(X\right)\right)\left(Y-E\left(Y\right)\right)\right], \\
& = \sum_{i} p_{i} \left(x_{i} - E\left( X \right)\right) \left(y_{i} - E\left( Y \right)\right).
\end{aligned}$$ $$\begin{aligned}
\operatorname{Correlation}\left(X,Y\right) = \rho_{X,Y} = \frac{\operatorname{Covariance}\left(X,Y\right)}{\sqrt{\operatorname{Variance}\left(X\right)\operatorname{Variance}\left(Y\right)}} = \frac{\sigma_{X,Y}}{\sigma_{X}\sigma_{Y}}.
\end{aligned}$$ $$\begin{aligned}
\operatorname{Beta}_{X}^{Y} = \beta_{X,Y} = \frac{\operatorname{Covariance}\left(X,Y\right)}{\operatorname{Variance}\left(Y\right)} = \frac{\sigma_{X,Y}}{\sigma_{Y}^{2}}.
\end{aligned}$$
...
## Computing the covariance and correlation {.smaller}

- Computation of the covariance with historical returns
  $$
  \text{Cov}\left(R_{i},R_{j}\right) = \frac{1}{T-1} \sum^{T}_{t=1}\left(R_{it} - \overline{R}_{i}\right)\left(R_{jt} - \overline{R}_{j}\right).
  $$

- The correlation is $$
  \text{Corr}\left(R_{i},R_{j}\right) = \frac{\text{Cov}\left(R_{i},R_{j}\right) }{\sigma_{R_{i}} \sigma_{R_{j}}}.
  $$

- **Reminder**: computation of expected returns and variance from
  historical data $$\begin{aligned}
  \overline{R}  & = \frac{1}{T} \sum^{T}_{t=1} R_{t} = \frac{1}{T}\left(R_{1} + \dots + R_{T} \right), \\
  \text{Var}\left[ R \right] & = \frac{1}{T-1} \sum^{T}_{t=1} \left(R_{t} - \overline{R}\right)^{2}.
  \end{aligned}$$
...
::: {.box .box-a}
$\rho_{12} = -1$ $\Rightarrow$
$\sigma_{p} = \left|x_{1}\sigma_{1}-x_{2}\sigma_{2}\right|$ Portfolio
weights $x_1$ and $x_2$ exist which eliminate the risk.
:::
...
## But wait\dots ``an efficient portfolio''? {.smaller}
...
## C. $N$ risky stocks: the efficient frontier {.smaller}
